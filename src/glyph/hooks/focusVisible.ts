...
export const useFocus = <T extends HTMLElement>(
	...
	const onBlur: JSX.FocusEventHandlerUnion<T, FocusEvent> = (event) => {
		if (!document.hasFocus() && focusVisible()) {
			// If tab is losing focus, we want to save the current state of the focusVisible.
			// so when we are back, and the element is focused again, we know that it was focused
			// with the keyboard.
			wasControlKeyPressed = true
		}
