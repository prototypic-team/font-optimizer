...
export const idbPutBlob = async (fontId: string, data: ArrayBuffer): Promise<void> => {
	const db = await getDb();
	return new Promise((resolve, reject) => {
		const tx = db.transaction("blobs", "readwrite");
		tx.oncomplete = () => resolve();
		tx.onerror = () => reject(tx.error ?? new Error("blob write failed"));
		tx.objectStore("blobs").put(data, fontId);
	});
};

export const idbHasBlob = async (fontId: string): Promise<boolean> => {
	const db = await getDb();
	return new Promise((resolve, reject) => {
		const tx = db.transaction("blobs", "readonly");
		const req = tx.objectStore("blobs").getKey(fontId);
		req.onerror = () => reject(req.error ?? new Error("blob key read failed"));
		req.onsuccess = () => resolve(req.result !== undefined);
	});
};

export const idbGetBlob = async (fontId: string): Promise<ArrayBuffer | undefined> => {
	const db = await getDb();
	return new Promise((resolve, reject) => {
		const tx = db.transaction("blobs", "readonly");
		const req = tx.objectStore("blobs").get(fontId);
...
export const idbDeleteBlob = async (fontId: string): Promise<void> => {
	const db = await getDb();
	return new Promise((resolve, reject) => {
		const tx = db.transaction("blobs", "readwrite");
		tx.oncomplete = () => resolve();
		tx.onerror = () => reject(tx.error ?? new Error("blob delete failed"));
		tx.objectStore("blobs").delete(fontId);
	});
};
